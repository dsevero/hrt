...
def pct_plot(intervals, vals, center=None):
    ...
    with sns.axes_style("white", {"legend.frameon": True}):
        ...
        ax = sns.heatmap(
            vals,
            cmap="plasma",
            xticklabels=50 + intervals,
            yticklabels=50 - intervals,
            center=center,
        )
        ax.invert_yaxis()
...
def main():
    ...
    for trial in range(ntrials):
        ...
        if np.any(np.isnan(linear_p_values[trial])):
            for feature in range(P):
                if np.any(np.isnan(linear_p_values[trial, feature])):
                    P_LINEAR_PATH = "data/{}/sweep_robust_linear_p_value_{}.npy".format(
                        trial, feature
                    )
                    linear_p_values[trial, feature] = (
                        np.load(P_LINEAR_PATH)
        ...
        NONLINEAR_P_VALUE_PATH = "data/{}/sweep_robust_nonlinear_p_values.npy".format(
            trial
        )
        ...
        if np.any(np.isnan(nonlinear_p_values[trial])):
            for feature in range(P):
                if np.any(np.isnan(nonlinear_p_values[trial, feature])):
                    P_NONLINEAR_PATH = "data/{}/sweep_robust_nonlinear_p_value_{}.npy".format(
                        trial, feature
                    )
                    nonlinear_p_values[trial, feature] = (
                        np.load(P_NONLINEAR_PATH)
        ...
        linear_tpr[trial], linear_fdr[trial] = p_tpr_fdr(
            linear_p_values[trial], intervals, truth, fdr_threshold
        )
        nonlinear_tpr[trial], nonlinear_fdr[trial] = p_tpr_fdr(
            nonlinear_p_values[trial], intervals, truth, fdr_threshold
        )
        # print(pretty_str(nonlinear_p_values[:,0,0]))
        # print(pretty_str(nonlinear_fdr))

    print(
        "*** Linear model ({} samples) ***".format(
            np.sum([np.all(~np.isnan(t)) for t in linear_tpr])
        )
    )
    print(
        "TPR:\n{} std: {}".format(
            pretty_str(np.nanmean(linear_tpr, axis=0) * 100),
            pretty_str(np.nanstd(linear_tpr, axis=0) * 100),
        )
    )
    print(
        "FDR:\n{} std: {}".format(
            pretty_str(np.nanmean(linear_fdr, axis=0) * 100),
            pretty_str(np.nanstd(linear_fdr, axis=0) * 100),
        )
    )
    print("")
    print(
        "*** Nonlinear model ({} samples) ***".format(
            np.sum([np.all(~np.isnan(t)) for t in nonlinear_tpr])
        )
    )
    print(
        "TPR:\n{} std: {}".format(
            pretty_str(np.nanmean(nonlinear_tpr, axis=0) * 100),
            pretty_str(np.nanstd(nonlinear_tpr, axis=0) * 100),
        )
    )
    print(
        "FDR:\n{} std: {}".format(
            pretty_str(np.nanmean(nonlinear_fdr, axis=0) * 100),
            pretty_str(np.nanstd(nonlinear_fdr, axis=0) * 100),
        )
    )
    print("")
    ...
    plt.savefig(
        "plots/liang-linear-alternative-p-values-zoomed.pdf", bbox_inches="tight"
    )
    p_plot(nonlinear_p_values[:, :S], intervals, start=0, end=0.1)
    plt.savefig(
        "plots/liang-nonlinear-alternative-p-values-zoomed.pdf", bbox_inches="tight"
    )
    p_plot(linear_p_values[:, S:], intervals, start=0, end=0.1)
