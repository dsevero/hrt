...
def accumulation_test(pvals, hfun, alpha=0.2, numerator_plus=0, denominator_plus=0):
    ...
    fdp_est = (numerator_plus + np.cumsum(hfun(pvals))) / (
        denominator_plus + 1.0 + np.arange(n)
    )
...
def hinge_exp(pvals, alpha=0.2, C=2):
    return accumulation_test(
        pvals,
        lambda x: C * np.log(1.0 / (C * (1 - x))) * (x > (1 - 1.0 / C)),
        alpha=alpha,
    )
...
if __name__ == "__main__":
    ...
    for info, order, p, n, c in zip(
        ...
        plt.savefig(
            "plots/predictors-order-vs-p-{}.pdf".format(info.prefix),
            bbox_inches="tight",
        )
        plt.close()
