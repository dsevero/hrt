...
class NeuralModel(nn.Module):
    def __init__(
        self, nfeatures, X_means, X_stds, y_mean, y_std, model_type="nonlinear"
    ):
...
def fit_nn(
    ...
    tY = autograd.Variable(
        torch.FloatTensor((y - y.mean()) / y.std()), requires_grad=False
    )
    ...
    model = NeuralModel(
        X.shape[1], X.mean(axis=0), Xstd, y.mean(), y.std(), model_type=model_type
    )
