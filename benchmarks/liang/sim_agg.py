...
if __name__ == "__main__":
    ...
    perm_p_values, linear_p_values, nonlinear_p_values = (
        np.full((ntrials, P), np.nan),
        np.full((ntrials, P), np.nan),
        np.full((ntrials, P), np.nan),
    )
    cv_perm_p_values, cv_linear_p_values, cv_nonlinear_p_values = (
        np.full((ntrials, P), np.nan),
        np.full((ntrials, P), np.nan),
        np.full((ntrials, P), np.nan),
    )
    (
        ...
        cv_nonlinear_tpr,
        cv_nonlinear_fdr,
    ) = (np.full(ntrials, np.nan) for _ in range(12))

    # Robust testing results
    sweep_robust_linear_p_values = np.full(
        (ntrials, P, intervals.shape[0], intervals.shape[0]), np.nan
    )
    sweep_robust_nonlinear_p_values = np.copy(sweep_robust_linear_p_values)
    (
        sweep_robust_linear_tpr,
        sweep_robust_linear_fdr,
        sweep_robust_nonlinear_tpr,
        sweep_robust_nonlinear_fdr,
    ) = (
        np.full((ntrials, intervals.shape[0], intervals.shape[0]), np.nan)
        for _ in range(4)
    )
    for trial in range(ntrials):
        ...
        SWEEP_ROBUST_CV_P_LINEAR_PATH = "data/{}/sweep_robust_linear_p_values.npy".format(
            trial
        )
        SWEEP_ROBUST_CV_P_NONLINEAR_PATH = "data/{}/sweep_robust_nonlinear_p_values.npy".format(
            trial
        )
        ...
        try:
            ...
            if os.path.exists(SWEEP_ROBUST_CV_P_LINEAR_PATH):
                sweep_robust_linear_p_values[trial] = np.load(
                    SWEEP_ROBUST_CV_P_LINEAR_PATH
                )
            if os.path.exists(SWEEP_ROBUST_CV_P_NONLINEAR_PATH):
                sweep_robust_nonlinear_p_values[trial] = np.load(
                    SWEEP_ROBUST_CV_P_NONLINEAR_PATH
                )
        ...
        add_if_finished(
            trial, linear_p_values, linear_tpr, linear_fdr, truth, fdr_threshold
        )
        add_if_finished(
            trial,
            nonlinear_p_values,
        ...
        add_if_finished(
            trial, cv_perm_p_values, cv_perm_tpr, cv_perm_fdr, truth, fdr_threshold
        )
        add_if_finished(
            trial,
            cv_linear_p_values,
        ...
        for i in range(intervals.shape[0]):
            for j in range(intervals.shape[0]):
                add_if_finished(
                    trial,
                    sweep_robust_linear_p_values[:, :, i, j],
                    sweep_robust_linear_tpr[:, i, j],
                    sweep_robust_linear_fdr[:, i, j],
                    truth,
                    fdr_threshold,
                )
                add_if_finished(
                    trial,
                    sweep_robust_nonlinear_p_values[:, :, i, j],
                    sweep_robust_nonlinear_tpr[:, i, j],
                    sweep_robust_nonlinear_fdr[:, i, j],
                    truth,
                    fdr_threshold,
                )
    ...
    print(
        "Heldout data randomization test (linear) ({} trials)".format(
            (~np.isnan(linear_tpr)).sum()
        )
    )
    ...
    print(
        "Heldout data randomization test (nonlinear) ({} trials)".format(
            (~np.isnan(nonlinear_tpr)).sum()
        )
    )
    ...
    print(
        "Cross-validation permutation test ({} trials)".format(
            (~np.isnan(cv_perm_tpr)).sum()
        )
    )
    ...
    print(
        "Cross-validation randomization test (linear) ({} trials)".format(
            (~np.isnan(cv_linear_tpr)).sum()
        )
    )
    ...
    print(
        "Cross-validation randomization test (nonlinear) ({} trials)".format(
            (~np.isnan(cv_nonlinear_tpr)).sum()
        )
    )
    ...
    print(
        "Robust Cross-validation randomization test (linear) ({} trials)".format(
            (~np.isnan(sweep_robust_linear_tpr)).sum(axis=0)[0, 0]
        )
    )
    print(
    ...
    print(
        "Cross-validation randomization test (nonlinear) ({} trials)".format(
            (~np.isnan(sweep_robust_nonlinear_tpr)).sum(axis=0)[0, 0]
        )
    )
    print(
        "TPR:\n{}".format(
            pretty_str(np.nanmean(sweep_robust_nonlinear_tpr, axis=0) * 100)
        )
    )
    print(
        "FDR:\n{}".format(
            pretty_str(np.nanmean(sweep_robust_nonlinear_fdr, axis=0) * 100)
        )
    )
    print("")
    sys.stdout.flush()
