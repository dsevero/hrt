...
def fit_elastic_net_ccle(X, y, nfolds=2):
    ...
    #                     cv=nfolds)
    enet = ElasticNetCV(
        l1_ratio=0.2,  # It always chooses l1_ratio=0.2
        alphas=np.exp(np.linspace(-6, 5, 250)),
        cv=nfolds,
...
def load_ccle(feature_type="expression", drug_target=None, normalize=False):
    if feature_type in ["expression", "both"]:
        ...
        expression = pd.read_csv(
            "data/expression.txt", delimiter="\t", header=2, index_col=1
        ).iloc[:, 1:]
    ...
    if feature_type in ["mutation", "both"]:
        ...
        mutations = pd.read_csv(
            "data/mutation.txt", delimiter="\t", header=2, index_col=1
        ).iloc[:, 1:]
    ...
    if feature_type == "both":
        ...
        both_df = pd.DataFrame(
            z, index=[c for c in expression.index] + [c for c in mutations.index]
        )
        features = both_df
    ...
    if normalize:
        X_drugs = [
            (
                x_i
                if (len(x_i) == 0)
                else (x_i - x_i.mean(axis=0, keepdims=True))
                / x_i.std(axis=0).clip(1e-6)
            )
            for x_i in X_drugs
        ]
        y_drugs = [
            (
                y_i
                if (len(y_i) == 0 or y_i.std() == 0)
                else (y_i - y_i.mean()) / y_i.std()
            )
            for y_i in y_drugs
        ]
...
def ccle_feature_filter(X, y, threshold=0.1):
    ...
    corrs = np.array(
        [np.abs(np.corrcoef(x, y)[0, 1]) if x.std() > 0 else 0 for x in X.T]
    )
...
def print_top_features(model, expected):
    ...
    for gene_target in expected:
        print(
            "{}: {}".format(
                gene_target, model_weights[ccle_features.get_loc(gene_target)]
            )
        )
    print("")
    print("Top by fit:")
...
if __name__ == "__main__":
    ...
    for rank, feat_idx in enumerate(top_features):
        gene_target = ccle_features[feat_idx]
        print(
            "{}. {}:\t\tweight={:.4f} p={:.4f}".format(
                rank + 1, gene_target, model_weights[top], p_values[rank]
            )
        )
