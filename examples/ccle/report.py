...
if __name__ == "__main__":
    ...
    for feat_idx in range(len(ccle_features)):
        ...
        if os.path.exists(p_path):
            ...
            print(
                "p-value for {}: {}".format(ccle_features[feat_idx], p_values[feat_idx])
            )
            continue
    ...
    for rank, (feat_idx, importance) in enumerate(
        zip(top_features, np.abs(model_weights)[top_features])
    ):
        p_value = p_values[feat_idx]
        print(
            "{}. {} importance={:.4f} p={:.4f}".format(
                rank + 1,
                ccle_features[feat_idx].replace("_MUT", " Mut"),
                importance,
                p_value,
            )
        )
    ...
    discoveries = np.arange(len(ccle_features))[important][
        bh(p_important, fdr_threshold)
    ...
    discovery_ranks[np.argsort(np.abs(model_weights))[::-1]] = (
        np.arange(len(ccle_features)) + 1
    )
    ...
    for r, g, w, p in zip(
        ...
        print(
            "{} & {} & {:.4f} & {:.4f} \\\\".format(g.replace("_MUT", " Mut"), r, w, p)
        )
